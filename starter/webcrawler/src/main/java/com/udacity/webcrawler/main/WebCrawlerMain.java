...
public final class WebCrawlerMain {
  ...
  private void run() throws Exception {
    ...
    if(!config.getResultPath().isEmpty()){
      ...
      System.out.println("HoangTQ test");
      Writer outputStreamWriter = new OutputStreamWriter(System.out);
        resultWriter.write(outputStreamWriter);

    }
    // TODO: Write the profile data to a text file (or System.out if the file name is empty)
    if (Strings.isNullOrEmpty(config.getProfileOutputPath())) {
      try(Writer profilerOutWriter  = new OutputStreamWriter(System.out)){
        profiler.writeData(profilerOutWriter);
      }
    }else{
